...
int AnalyzeSignal(const std::string &default_config_filename,
   ...
   if (!user_config_filename.empty() &&
       ReadConfig(user_config_filename.c_str(), false, &params) == EXIT_FAILURE)
      return EXIT_FAILURE;
   return AnalyzeSignalWithParams(signal, &params, data);
}
...
int RunSynthesis(const std::string &filename,
   ...
   FftFilterExcitation(params, data.fundf, data.frame_energy, data.spectrum,
                       data.lsf_vocal_tract, data.lsf_glot,
                       data.excitation_signal, &(data.signal));
