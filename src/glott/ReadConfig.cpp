...
int AssignConfigParams(const libconfig::Config &cfg, const bool required, Param *params) {
	...
	if (ConfigLookupBool("USE_VELVET_UNVOICED_PAF", cfg, false, &(params->use_velvet_unvoiced_paf)) == EXIT_FAILURE)
		return EXIT_FAILURE;
	...
	if( required || str != "") {
	   if (str == "SINGLE_PULSE")
         ...
         params->excitation_method = IMPULSE_EXCITATION;
	      ...
	      std::cerr << "Error: invalid excitation method flag \"" << str << "\"" << std::endl;
	      std::cerr << "Valid options are SINGLE_PULSE / DNN_GENERATED / PULSES_AS_FEATURES / EXTERNAL/ IMPULSE" << std::endl;
	      return EXIT_FAILURE;
	   }
	}
